...
// Move the carousel (scroll) by the given direction (-1 for left, 1 for right)
function moveCarousel(direction) {
    const carouselInner = document.getElementById('carouselInner');
    const itemWidth = carouselInner.children[0].offsetWidth; // Get width of a carousel item
    carouselInner.scrollBy({ left: direction * itemWidth, behavior: 'smooth' });
}
...
$(document).ready(function() {
    ...
    $('#bookButton').click(function(event) {
        ...
        var fromValue = $('#fromDropDown option:selected').text();
        var toValue = $('#toDropdown option:selected').text();
